...
function showSpinner() {
  ...
  el.className =
    "fixed inset-0 z-50 grid place-items-center bg-black/30 backdrop-blur-sm";
  el.innerHTML =
    '<div class="animate-spin h-10 w-10 rounded-full border-4 border-brand border-t-transparent"></div>';
...
function log(result: "success" | "timeout" | "error", detail?: string) {
  ...
  try {
    if (env.analyticsUrl && "sendBeacon" in navigator)
      navigator.sendBeacon(env.analyticsUrl, JSON.stringify(payload));
  } catch {
    // Silently ignore analytics errors
  }
  console[result === "success" ? "info" : "warn"](
    "[jobsRedirect]",
    result,
    detail || "",
  );
...
function targetUrl(el: Element): string {
  ...
  const token =
    (window as { __APP_AUTH_TOKEN?: string }).__APP_AUTH_TOKEN || "";
...
function handler(e: Event) {
  ...
  setTimeout(() => {
    log("success", `${Math.round(performance.now() - start)}ms`);
  }, 600);
}

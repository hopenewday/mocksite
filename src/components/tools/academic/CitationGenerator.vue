  ...
  <div class="card card-brutal-cyan p-8">
    ...
    <div class="grid md:grid-cols-3 gap-6 mb-8">
      ...
      <div class="bg-brutal-white dark:bg-brutal-black border-4 border-black dark:border-brutal-white p-6 space-y-4">
        ...
        <button
          class="btn-primary btn-primary-yellow w-full text-sm"
          @click="addToBib"
        >
          + Add to Bibliography
        </button>
    ...
    <!-- Generated Citation -->
    <div
      v-if="generatedCitation"
      class="bg-brutal-cyan border-4 border-black p-6 mb-6"
    >
      <div class="text-sm font-bold text-brutal-gray mb-2">
        Generated Citation
      </div>
      <div class="font-bold italic p-4 bg-brutal-white border-2 border-black">
        {{ generatedCitation }}
      </div>
    </div>
    ...
    <div class="bg-brutal-pink border-4 border-black p-6">
      ...
      <div class="space-y-2 max-h-64 overflow-y-auto">
        <div
          v-for="(cit, idx) in bibliography"
          :key="idx"
          class="bg-brutal-white border-2 border-black p-3 italic text-sm"
        >
          <div>{{ cit }}</div>
          <button
            class="text-xs btn-small btn-primary-pink mt-2"
            @click="removeCitation(idx)"
          >
            Remove
          </button>
        </div>
        <div
          v-if="bibliography.length === 0"
          class="text-sm font-bold text-brutal-gray py-4"
        >
          No citations yet.
        </div>
      </div>

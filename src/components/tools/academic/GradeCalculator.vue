<template>
  <div class="card card-brutal-pink p-8">
    <h2 class="subheading-brutal mb-6">Grade Calculator</h2>
    ...
    <div class="grid md:grid-cols-3 gap-6 mb-8">
      ...
      <div class="bg-brutal-white border-4 border-black p-6 space-y-4 md:col-span-1">
        ...
        <button class="btn-primary btn-primary-cyan w-full" @click="calculateNeeded">
          Calculate Grade Needed
        </button>

        <div class="grid grid-cols-2 gap-2">
          <button class="btn-primary btn-primary-lime" :class="{ 'animate-copy-bounce': copying }" @click="copyGrade">
            {{ copying ? 'Copied!' : 'Copy' }}
          </button>
          <button class="btn-primary btn-primary-yellow" @click="clearAssignments">Clear All</button>
        </div>
      </div>

      <!-- Assignments -->
      <div class="md:col-span-2 bg-brutal-cyan border-4 border-black p-6">
        <h3 class="font-black mb-4">Assignments & Tests</h3>
        <div class="space-y-3 max-h-96 overflow-y-auto">
          <div v-for="(item, idx) in assignments" :key="idx" class="bg-brutal-white border-2 border-black p-3 grid grid-cols-4 gap-2 items-end">
            <input v-model="item.name" type="text" placeholder="Name" class="input text-xs col-span-2">
            <input v-model.number="item.score" type="number" min="0" max="100" placeholder="Score" class="input text-xs">
            <button class="btn-small btn-primary-pink text-xs" @click="removeAssignment(idx)">✕</button>
          </div>
        </div>
        <button class="btn-primary btn-primary-lime w-full mt-4" @click="addAssignment">+ Add Assignment</button>
      </div>
    </div>
    ...
    <div class="grid md:grid-cols-3 gap-6">
      <div class="bg-brutal-yellow border-4 border-black p-6">
        <div class="text-sm font-bold text-brutal-gray mb-2">Current Grade</div>
        <div class="text-4xl font-black">{{ currentGrade.toFixed(1) }}%</div>
        <div class="text-sm font-bold mt-2 text-brutal-gray">{{ getLetterGrade(currentGrade) }}</div>
      </div>

      <div class="bg-brutal-lime border-4 border-black p-6">
        <div class="text-sm font-bold text-brutal-gray mb-2">Grade Needed on Final</div>
        <div class="text-4xl font-black">{{ neededGrade.toFixed(1) }}%</div>
        <div v-if="neededGrade > 100" class="text-sm text-red-600 font-bold mt-2">❌ Impossible</div>
        <div v-else-if="neededGrade > 80" class="text-sm text-orange-600 font-bold mt-2">⚠️ Difficult</div>
        <div v-else class="text-sm text-green-600 font-bold mt-2">✓ Achievable</div>
      </div>

      <div class="bg-brutal-pink border-4 border-black p-6">
        <div class="text-sm font-bold text-brutal-gray mb-2">Predicted Final Grade</div>
        <div class="text-4xl font-black">{{ predictedGrade.toFixed(1) }}%</div>
        <div class="text-sm font-bold mt-2 text-brutal-gray">{{ getLetterGrade(predictedGrade) }}</div>
      </div>
    </div>
    ...
    <div class="mt-8 bg-brutal-white border-4 border-black p-6">
      ...
      <div class="w-full bg-brutal-gray border-2 border-black h-6">
        <div class="bg-brutal-cyan border-2 border-black h-6" :style="{ width: `${Math.min(100, currentGrade)}%` }"></div>
      </div>
    </div>
  </div>
</template>
